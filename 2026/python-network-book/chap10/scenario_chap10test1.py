...
network_event_scheduler = NetworkEventScheduler(
    log_enabled=True, verbose=True, stp_verbose=False, tcp_verbose=True
)
...
node2.start_tcp_traffic(
    destination_url="192.168.1.0/24",
    bitrate=5000,
    start_time=2.0,
    duration=2.0,
    header_size=50,
...
tcp_logs = {
    pid: log
    for pid, log in network_event_scheduler.packet_logs.items()
    if log["packet_type"] == "TCPPacket"
}

network_event_scheduler.generate_summary(tcp_logs)
